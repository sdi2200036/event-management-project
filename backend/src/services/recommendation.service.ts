		...
		for (let epoch = 0; epoch < this.numEpochs; epoch++) {
			...
			for (const { userId, eventId, rating } of shuffled) {
				...
				this.userFactors.set(
					userId,
					p_u.map((val, k) => val + this.learningRate * (error * q_i[k] - this.regularization * val))
				);
				this.itemFactors.set(
					eventId,
					q_i.map((val, k) => val + this.learningRate * (error * p_u[k] - this.regularization * val))
				);
			}
		}
	}
...
export const trainModel = async (): Promise<BiasedMatrixFactorization> => {
	const [bookings, views] = await Promise.all([
		prisma.booking.findMany({
			...
			select: { attendee_id: true, event_id: true }
		}),
...
export const getRecommendationsForUser = async (userId: number, topN: number = 10): Promise<number[]> => {
	...
	const [bookedIds, viewedIds] = await Promise.all([
		prisma.booking
			.findMany({ where: { attendee_id: userId }, select: { event_id: true } })
			.then((r) => r.map((b) => b.event_id)),
		prisma.eventView
			.findMany({ where: { user_id: userId }, select: { event_id: true } })
			.then((r) => r.map((v) => v.event_id))
	]);
